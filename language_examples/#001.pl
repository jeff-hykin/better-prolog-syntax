...
% Norvig's "Artificial Intelligence: A Modern Approach", Section 7.2.
...
%   1,1.  World can be either 'fig72' for Figure 7.2 of Russell and Norvig,
...
%   fig72, which generates the wumpus world in Figure 7.2 of [Russell &
...
%WNb
  ...
  random(IShouldBeRandom),
  write('random is: '), write(IShouldBeRandom), write('\n'),
  ...
  length(AllSqrs,LengthOfWumpusPossibilites),
  WumpusIndex = ceiling(random(RandomNumberForWumpus) * LengthOfWumpusPossibilites),
  % get the coordinates
  nth1(WumpusIndex, AllSqrs, [WX,WY]),
  random_member([WX,WY],AllSqrs1),  % initialize wumpus
  write('Wumpus location is '),write(WX),write(' '),write(WY),write('\n'),
